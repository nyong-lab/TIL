...
몇개의 화면에서 공통으로 보여줄 수 있기 때문에 빠르게 메뉴 기능에 접근하고자 할 때 사용하고, 로그인한 사용자의 프로필 정보나 설정 메뉴를 보여줄 때도 사용할 수 있다. 

바로가기 메뉴를 추가하는 가장 쉬운 방법은 안드로이드에서 첫 화면의 유형으로 제공하는 것을 사용하는 것인데, 새로운 프로젝트를 만들때 `Navigation Drawer Activity` 를 선택하면 된다.
